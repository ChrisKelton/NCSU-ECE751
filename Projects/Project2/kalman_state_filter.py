...
def plot_kalman_filter_estimates_vs_observations_and_ground_truth(
    ...
    plt.plot(x, ground_truth, label="truth")
    plt.plot(x, observations, "-.", label="observations")
    plt.plot(x, estimates, "--", label="estimates")
    plt.title("Kalman Filter Discrete-Time, Time-Invariant", fontsize=9)
    plt.xlabel("Time (s)")
    if xticks is None:
        xticks = x
    ...
    plt.legend(["truth", "observations", "estimates"])
...
def plot_kalman_filter_kalman_gain_position_and_velocity(
    K: np.ndarray,  # K[0] = position, K[1] = velocity
    fig_output_path: Path,
    xticks: Optional[Union[list, np.ndarray]] = None,
):
    x = np.arange(0, K.shape[1])
...
def plot_kalman_filter_estimate_covariance_diagonals(
    P: np.ndarray,  # P[0] = diagonal elements of position, P[1] = diagonal elements of velocity
    fig_output_path: Path,
    xticks: Optional[Union[list, np.ndarray]] = None,
):
...
def roughly_constant_velocity_motion_filter(
    ...
    if fig_output_path is not None:
        ...
        plot_kalman_filter_estimate_covariance_diagonals(
            P=np.diagonal(P, axis1=1, axis2=2).T,
            fig_output_path=temp_path,
            xticks=xticks,
        )
